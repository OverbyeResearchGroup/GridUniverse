<template>
	<v-card>
		...
		<v-card-text class="pa-0">
			<template>
				<v-data-table class="fixed-header" :headers="headers" :items="genData" :rows-per-page-items="defaultRowItems" v-model="selected" select-all disable-initial-sort item-key="name">
		...
		updateTable() {
			...
			try {
				...
				for (let i in this.gens) {
					...
					if (
							this.$store.state.genAction['Gen'][this.gens[i].key] !=
							undefined
						) {
							if (
